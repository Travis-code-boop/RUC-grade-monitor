...
def describe_login_error(error_description: str) -> str:
    if error_description == "the username and the password can't be empty":
        return "统一身份认证账号或密码为空"
    ...
    if error_description == "the user is not found":
        return "统一身份认证用户不存在"
    if "please try again after" in error_description:
        return "统一身份认证失败次数过多，账号被临时锁定"
    if "login have chances" in error_description:
        return "统一身份认证账号或密码不正确，剩余尝试次数有限"
...
def _request_text(
    ...
    raise RucPasswordLoginError("统一身份认证网络请求失败")
